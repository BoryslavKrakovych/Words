...
#
# Name          : Krakovych Boryslav
# Collaborators : No one
# Time spent    : start-03.12.2021; finish-06.12.2021-4 days
...
def is_valid_word(word, hand, word_list):
    ...
    for letter in word:
        if letter not in string.ascii_letters and letter != '*':
            return False
        if freq_word.get(letter, 0) > hand.get(letter, 0):
            return False
    return True
#
...
def substitute_hand(hand, letter):
    ...
    if new_letter in hand:
        while new_letter in hand:
            new_letter = random.choice(VOWELS + CONSONANTS)
    elif new_letter not in hand:
        new_hand = {}
        for key in hand:
            if key == letter:
                new_hand[new_letter] = hand[key]
            else:
    ...
    for i in range(total_hands):
        ...
        if letter_substituted >= 0:
            ...
            if substitute.lower() == 'yes':
                ...
                letter_substituted = letter_substituted + 1
